	...
	$.widget( 'wikibase.snakview', PARENT, {
		...
		options: {
			...
			templateShortCuts: {
				...
				$cloneBtn: '.wikibase-snakview-clonebtn'
			},
			value: {
		...
		_buildPropertySelector: function () {
			...
			input.on( 'input' , function ( event ) {
				if (!input.value) {
					self._isCloneBtnEnabled = false;
					self.drawEntityCloneBtn();
				}
			});
			...
			.on( 'entityselectorselected', function ( event, entityId ) {
				...
				var selectedItemData = self._getPropertySelector()._selectedEntity;
				// console.log("source:",selectedItemData.repository.toLowerCase());
				if (selectedItemData.repository.toLowerCase() !== "local") {
					//externally sourced data to be clone
					self._isCloneBtnEnabled = true;

					self._item_id = selectedItemData.id;
					self._item_description = selectedItemData.description;
				}else{
					self._isCloneBtnEnabled = false;
				...
				// self._getPropertySelector()._selectedEntity
				self.drawEntityCloneBtn();
		...
		drawEntityCloneBtn: function () {
			var self = this;
			var firstChild = this.$cloneBtn.children().first();
			var isBtnMounted = firstChild.length > 0;
			...
			if ( isBtnMounted ) {
				...
				//btn = $('<button>Clone</button>')
				btn = $("<button>Clone</button>")
				this.$cloneBtn.append(btn);
			}
			...
			btn.on('click', function () {
				...
				var full_endpoint = WIKIBASE_SYNC_URL + 'import-wikidata-item/' + self._item_id;
				$.ajax({
					url: full_endpoint,
					crossDomain: true,
					headers: {
