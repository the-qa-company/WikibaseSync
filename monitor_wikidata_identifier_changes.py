...
class MonitorChanges:
    ...
    def get_claim(self, item_id):
        ...
        claims = entity.get("claims")  # Get all the existing claims
        return claims
    ...
    def check_differences(self, item_id, change):
        if item_id and item_id[0] == "Q":
            ...
            if (
                "" + self.wikidata_code_property_id + "" in existing_claims["claims"]
                and len(list(existing_claims.get("claims"))) > 1
            ):
                rev_text = []
                ...
                if (
                    json.loads(rev_text[0])
                    .get("claims")
                    .get(self.wikidata_code_property_id, None)
                    is not None
                    and json.loads(rev_text[1])
                    .get("claims")
                    .get(self.wikidata_code_property_id, None)
                    is None
                ):
                    wikidata_qid = existing_claims["claims"][
                        self.wikidata_code_property_id
                    ][0].toJSON()["mainsnak"]["datavalue"]["value"]
                    print(
                        "Entity "
                        + item_id
                        + " has a new link to wikidata id "
                        + wikidata_qid
                        + " importing it ... "
                    )
                    wikidata_item = pywikibot.ItemPage(self.wikidata_repo, wikidata_qid)
                    wikidata_item.get()
                    self.wikibase_importer.change_item(
                        wikidata_item, self.wikibase_repo, True
                    )
                else:
                    return
    ...
    def get_changes(self):
        ...
        requests = self.wikibase.recentchanges(
            start=current_time, end=current_time - timedelta(minutes=20)
        )
