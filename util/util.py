...
print('Wikidata Property Identifier',identifier.propertyIdentifier)


endpoint = "http://query.linkedopendata.eu/bigdata/namespace/wdq/sparql"
id = IdSparql(endpoint, identifier.itemIdentifier, identifier.propertyIdentifier)
id.load()
print(id.mapEntity)
...
# comparing the descriptions
def change_descriptions(wikidata_item, wikibase_item):
    ...
    if len(myDescriptions) != 0:
        ...
        try:
            wikibase_item.editEntity({'descriptions': myDescriptions}, summary='The description in Wikidata changed')
        except pywikibot.exceptions.OtherPageSaveError as e:
...
def importItem(wikidata_item):
    ...
    mySitelinks = [];
    claim = pywikibot.page.Claim(wikibase_repo, identifier.itemIdentifier, datatype='external-id')
...
def importProperty(wikidata_item):
    ...
    myDescriptions = diffDescriptions(wikidata_item, wikibase_item)
    myaliases = diffAliases(wikidata_item, wikibase_item)
    claim = pywikibot.page.Claim(wikibase_repo, identifier.propertyIdentifier, datatype='external-id')
    target = wikidata_item.getID()
    claim.setTarget(target)
...
def compare_claim(wikidata_claim, wikibase_claim, translate):
    found = False
    found_equal_value = False
    wikidata_propertyId = wikidata_claim.get('property')
    wikibase_propertyId = wikibase_claim.get('property')
    if ((translate == True and id.get_id(wikidata_propertyId) == wikibase_propertyId) or (translate == False and wikidata_propertyId == wikibase_propertyId)):
        found = True
            ...
            # GLOBAL-COORDINATE
                ...
                if wikibase_claim.get('datatype') == 'globe-coordinate':
                    ...
                    if wikibase_latitude == wikidata_latitude and wikibase_longitude == wikidata_longitude and wikibase_globe == wikidata_globe\
                            and wikibase_altitude == wikidata_altitude and (wikibase_precision == wikidata_precision or (wikibase_precision==1 and wikidata_precision==None)) :
                        found_equal_value = True
            # QUANTITY
...
def translateClaim(wikidata_claim):
    ...
    if  wikidata_claim.get('snaktype') == 'somevalue':
        claim = pywikibot.Claim(wikibase_repo, id.get_id(wikidata_propertyId), datatype=wikidata_claim.get('datatype'))
        ...
        # QUANTITY
            ...
            if wikidata_unit == None or wikidata_unit == '1':
                if wikidata_upperBound == None:
                    ...
                    target = pywikibot.WbQuantity(amount='{:f}'.format(Decimal(wikidata_amount)), site=wikibase_repo)
                    claim.setTarget(target)
                    claim.setRank(wikidata_claim.get('rank'))
                    return claim
                else:
                    #print("Here 2", '{:f}'.format(Decimal(wikidata_amount)))
                    target = pywikibot.WbQuantity(amount=Decimal(wikidata_amount), site=wikibase_repo,
...
def compare_claim_with_qualifiers_and_references(wikidata_claim, wikibase_claim, translate):
    # compare mainsnak
    found = False
    found_equal_value = False
    (claim_found, main_claim_found_equal_value) = compare_claim(wikidata_claim.get('mainsnak'),wikibase_claim.get('mainsnak'),translate)
    # compare qualifiers
                ...
                for q2 in wikibase_claim.get('qualifiers'):
                    for q_wikibase in wikibase_claim.get('qualifiers').get(q2):
                        ...
                        if id.contains_id(wikidata_propertyId):
                            (qualifier_claim_found, qualifier_claim_found_equal_value) = compare_claim(q_wikidata, q_wikibase,translate)
    ...
    if ('references' in wikidata_claim and not('references' in wikibase_claim)) or (not('references' in wikidata_claim) and 'references' in wikibase_claim):
        references_equal = False
    if main_claim_found_equal_value and qualifiers_equal and references_equal and wikidata_claim.get('rank') ==  wikibase_claim.get('rank'):
        found_equal_value = True
    more_accurate = False
    ...
    if main_claim_found_equal_value and ('references' not in wikibase_claim) and ('qualifiers' not in wikibase_claim) and (('references' in wikidata_claim and len(wikidata_claim.get('references'))>0) or ('qualifiers' in wikidata_claim and len(wikidata_claim.get('qualifiers'))>0)):
        more_accurate = True
...
def changeClaims(wikidata_item,wikibase_item):
                    ...
                    if id.contains_id(wikidata_propertyId):
                        if id.get_id(wikidata_propertyId) == wikibase_propertyId:
                            ...
                            # if wikidata_propertyId == 'P2884':
                                ...
                                (found_here, found_equal_value_here, more_accurate_here) = compare_claim_with_qualifiers_and_references(wikidata_claim,wikibase_claim,True)
    ...
    if not is_only_wikidata_updater_user:
        for i in  range(0,len(claimsToRemove)):
            ...
            if claim_more_accurate[i] == False: # if the claim is more accurate it is better to cancel the existing one
                edit_where_claim_was_added = len(revisions)-1
                for i in range(0,len(revisions)):
                    #print("new revision ",revisions[i]['user'])
                    item_revision = jsonToItem(wikibase_repo, revisions[i]['text'])
                    found = False
                    for claims_revision in item_revision['claims']:
                        if found == False:
                            for c_revision in item_revision['claims'].get(claims_revision):
                                if found == False:
                                    (found_here, found_equal_value_here, more_accurate) = compare_claim_with_qualifiers_and_references(claimToRemove.toJSON(),c_revision.toJSON(),False)
    ...
    if len(claimsToRemove)>0:
        for claimsToRemoveChunk in chunks(claimsToRemove,50):
            wikibase_item.get()
            wikibase_item.removeClaims(claimsToRemoveChunk, summary="Removing this statements since they changed in Wikidata")
    ...
    if wikibase_item.getID().startswith("Q"):
        ...
        wikibase_item = pywikibot.PropertyPage(wikibase_repo, wikibase_item.getID())
    wikibase_item.get()
    newClaims = []
    for claims in wikidata_item.claims:
        for c in wikidata_item.claims.get(claims):
    ...
    if hasattr(wikibase_item, "claims"):
        for wikibase_claims in wikibase_item.claims:
            for wikibase_c in wikibase_item.claims.get(wikibase_claims):
                wikibase_claim = wikibase_c.toJSON()
                wikibase_propertyId = wikibase_claim.get('mainsnak').get('property')
                if wikibase_propertyId == identifier.itemIdentifier:
                    found = True
    if found == False:
        claim = pywikibot.page.Claim(wikibase_repo, identifier.itemIdentifier, datatype='external-id')
        target = wikidata_item.getID()
