...
query = (
    """
           ...
           #  } order by desc(?id)
            SELECT ?s1 ?id WHERE {
           ?s1  <"""
    + app_config.get("wikibase", "propertyUri")
    + """/direct/P1>  ?id .
   ...
   ?s1  <"""
    ...
    + app_config.get("wikibase", "propertyUri")
    + """/statement/P35> <"""
    + app_config.get("wikibase", "entityUri")
    + """/Q196899> .         
   ?s1  <"""
    + app_config.get("wikibase", "propertyUri")
    + """/direct/P1>  ?id .  
...
for result in results["results"]["bindings"]:
    ...
    try:
        ...
        query = (
            "select ?id where { <http://www.wikidata.org/entity/"
            + id
            + "> <http://www.w3.org/2002/07/owl#sameAs> ?id . }"
        )
        sparql.setQuery(query)
        sparql.setReturnFormat(JSON)
        new_results = sparql.query().convert()
        for new_result in new_results["results"]["bindings"]:
            newId = new_result["id"]["value"].replace(
            ...
            try:
                wikidata_item.get()
                wikibase_importer.change_item_given_id(
                    wikidata_item,
                    result["s1"]["value"].replace(
